...
unsigned long noCountStartTime = 0; // 카운트가 증가하지 않은 시간 추적
bool counting = false; // 카운팅 상태
bool resting = false; // 휴식 상태
unsigned long countLimit = 20000; // 20초 동안 카운팅
unsigned long restLimit = 10000; // 10초 동안 휴식
unsigned long noCountLimit = 7000; // 7초 동안 카운트가 늘어나지 않으면 멈추기
...
noCountStartTime = currentMillis; // 카운트가 증가하지 않으면 멈추기 타이머 시작
lcd.clear();
lcd.print("Set ");
...
// 두 버튼이 떼어지면 카운팅 중지, 삐 소리가 길게 나도록 처리
if ((digitalRead(BUTTON1_PIN) == HIGH || digitalRead(BUTTON2_PIN) == HIGH) && counting) {
counting = false;
tone(BUZZER_PIN, 1000, 3000); // 1000Hz, 2000ms 동안 소리
lcd.clear();
...
lcd.clear();
lcd.print("Ready!"); // Ready 화면으로 돌아가기
}
...
// 카운트가 증가했으면 noCountStartTime 리셋
noCountStartTime = currentMillis;
}

 

// 7초 동안 카운트가 증가하지 않으면 자동으로 카운팅 멈추기
if (currentMillis - noCountStartTime >= noCountLimit) {
counting = false;
tone(BUZZER_PIN, 1000, 3000); // 1000Hz, 2000ms 동안 소리
lcd.clear();
lcd.print("Counting Stopped");
